  ...
  () => {
    ...
    it("should report unknown publications", () => {
      return createBroker({
        ...
        return broker.publish("does-not-exist", "test message").catch((err) => {
          ...
          assert.strictEqual(
            err.message,
            "Unknown publication: does-not-exist"
          );
        });
      });
    });
      ...
      createBroker({
        ...
        broker.publish("p1", "test message").then((publication) => {
          publication.on("success", (messageId) => {
            assert.ok(
              /\w+-\w+-\w+-\w+-\w+/.test(messageId),
              format("%s failed to match expected pattern", messageId)
            );
    ...
    it("should publish json messages to normal exchanges", (test, done) => {
      createBroker({
        ...
        broker
          .publish("p1", { message: "test message" })
          .then((publication) => {
            publication.on("success", () => {
            ...
            publication.on("success", () => {
              amqputils.getMessage("q1", namespace, (err, message) => {
                ...
                assert.strictEqual(
                  ...
                  "application/vnd+custom.contentType.v1"
                );
                assert.strictEqual(
    ...
    it("should publish buffer messages to normal exchanges", (test, done) => {
      createBroker({
        ...
        broker
          .publish("p1", Buffer.from("test message"))
          .then((publication) => {
            publication.on("success", () => {
              amqputils.assertMessage("q1", namespace, "test message", done);
            });
    ...
    it("should allow publish overrides", (test, done) => {
      createBroker({
        ...
        broker
          .publish("p1", "test message", { options: { expiration: 1 } })
          .then((publication) => {
            publication.on("success", () => {
              setTimeout(() => {
            ...
            broker.forward("p2", message).then((publication) => {
              publication.on("success", () => {
                ...
                amqputils.getMessage("q2", namespace, (err, message) => {
                  ...
                  assert.strictEqual(
                    message.properties.headers.rascal.restoreRoutingHeaders,
                    false
                  );
                  assert.strictEqual(
        ...
        broker.publish("p1", "test message").then((publication) => {
          publication.on("success", (messageId) => {
            amqputils.getMessage("q1", namespace, (err, message) => {
              ...
              assert.strictEqual(
                32,
                message.properties.headers.rascal.encryption.iv.length
              );
              assert.strictEqual(
                "text/plain",
