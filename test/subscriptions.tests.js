      ...
      createBroker(
        ...
        (err, broker) => {
          ...
          broker.subscribe("does-not-exist", (err) => {
            ...
            assert.strictEqual(
              err.message,
              "Unknown subscription: does-not-exist"
            );
            done();
          });
        }
      ...
      createBroker(
        ...
        (err, broker) => {
          ...
          amqputils.publishMessage(
            ...
            { routingKey: "foo", contentType: "application/json" },
            (err) => {
              assert.ifError(err);
              broker.subscribe("s1", (err, subscription) => {
                assert.ifError(err);
        ...
        (err, broker) => {
          ...
          broker.publish("p1", "test message", (err) => {
            ...
            broker.subscribe("s1", (err, subscription) => {
              ...
              subscription.on("message", (message, content, ackOrNack) => {
                assert.ok(message);
                messages[message.properties.messageId] = messages[
                  message.properties.messageId
                ]
                  ? messages[message.properties.messageId] + 1
                  : 1;
        ...
        (err, broker) => {
          ...
          broker.publish("p1", "test message", (err) => {
            ...
            broker.subscribe("s1", (err, subscription) => {
              ...
              subscription
                .on("message", (message) => {
                  if (message.properties.headers.rascal.redeliveries >= 10)
                    return subscription.cancel(done);
                  throw new Error("oh no");
                })
                .on("error", () => {
                  if (errors++ > 10)
                    done(new Error("Redeliveries were not counted"));
                });
            });
          });
        }
          ...
          broker.publish("p1", "test message", (err) => {
            ...
            broker.subscribe("s1", (err, subscription) => {
              ...
              subscription
                ...
                .on("error", () => {
                  if (errors++ > 5)
                    done(new Error("Redeliveries were exceeded"));
                });
            });
          });
        }
          ...
          broker.publish("p1", "test message", (err) => {
            ...
            broker.subscribe("s1", (err, subscription) => {
              ...
              subscription
                ...
                .on("error", (err) => {
                  if (
                    !/Message .* has exceeded 5 redeliveries/.test(err.message)
                  )
                    return;
                  broker.shutdown((err) => {
                    assert.ifError(err);
          ...
          broker.publish("p1", "test message", (err) => {
            ...
            broker.subscribe("s1", (err, subscription) => {
              ...
              subscription.on("message", (message, content, ackOrNack) => {
                ...
                if (messages[message.properties.messageId] < 10)
                  return ackOrNack(
                    { message: "republish me", code: "red" },
                    { strategy: "republish" }
                  );
                assert.strictEqual(
                  message.properties.headers.rascal.recovery[
                    broker.qualify("/", "q1")
                  ].republished,
                  9
                );
                assert.strictEqual(
                  message.properties.headers.rascal.error.message,
                  "republish me"
                );
                assert.strictEqual(
                  message.properties.headers.rascal.error.code,
                  "red"
                );
                done();
              });
          ...
          broker.publish("p1", "test message", (err) => {
            ...
            broker.subscribe("s1", (err, subscription) => {
              ...
              subscription.on("message", (message, content, ackOrNack) => {
                ...
                assert.strictEqual(
                  message.properties.headers.rascal.error.message.length,
                  1024
                );
                done();
              });
        ...
        (err, broker) => {
          ...
          broker.subscribe("s1", (err, subscription) => {
            ...
            subscription.on("message", (message, content, ackOrNack) => {
              assert.ok(message);
              ackOrNack(
                { message: "forward me", code: "red" },
                { strategy: "forward", publication: "p2" }
              );
            });
          });
          ...
          broker.subscribe("s2", (err, subscription) => {
            ...
            subscription.on("message", (message, content, ackOrNack) => {
              ...
              assert.strictEqual(
                message.properties.headers.CC[0],
                broker.qualify("/", "q1") + ".bar"
              );
              assert.strictEqual(
                message.properties.headers.rascal.error.message,
                "forward me"
        ...
        (err, broker) => {
          ...
          broker.subscribe("s2", (err, subscription) => {
            ...
            subscription.on("message", (message, content, ackOrNack) => {
              ...
              assert.strictEqual(
                message.properties.headers.CC[0],
                broker.qualify("/", "q_10.10.10.10") + ".bar"
              );
              assert.strictEqual(
                message.properties.headers.rascal.error.message,
                "forward me"
                ...
                subscription
                  ...
                  .on("error", (err) => {
                    ...
                    assert.strictEqual(
                      "Message: " +
                        messageId +
                        " was forwared to publication: p3, but was returned",
                      err.message
                    );
                    done();
                  });
              ...
              broker.subscribe("s1", (err, subscription) => {
                ...
                subscription
                  ...
                  .on("error", (err) => {
                    assert.ok(err);
                    assert.strictEqual(
                      "Error recovering message: " +
                        messageId +
                        ". No such strategy: foo.",
                      err.message
                    );
                    done();
                  });
          ...
          broker.publish("p1", "test message", (err) => {
            ...
            broker.subscribe("s1", (err, subscription) => {
              ...
              subscription.on("message", (message, content, ackOrNack) => {
                ...
                ackOrNack(
                  new Error("retry"),
                  [{ strategy: "republish", attempts: 5 }, { strategy: "ack" }],
                  () => {
                    messages[message.properties.messageId] = messages[
                      message.properties.messageId
                    ]
                    ...
                    setTimeout(() => {
                      assert.strictEqual(
                        messages[message.properties.messageId],
                        6
                      );
                      done();
                    }, 500);
          ...
          broker.publish("p1", "test message", (err) => {
            ...
            broker.subscribe("s1", (err, subscription) => {
              ...
              subscription.on("message", (message) => {
                ...
                assert.strictEqual(
                  message.properties.headers.rascal.originalVhost,
                  "/"
                );
                done();
              });
            ...
            broker.subscribe("s1", (err, subscription) => {
              ...
              subscription
                ...
                .on("error", (err) => {
                  assert.strictEqual(
                    err.message,
                    "The channel has been closed. Unable to ack message"
                  );
                  done();
                });
            });
            ...
            broker.subscribe("s1", (err, subscription) => {
              ...
              subscription
                ...
                .on("error", (err) => {
                  assert.strictEqual(
                    err.message,
                    "The channel has been closed. Unable to nack message"
                  );
                  done();
                });
            });
            ...
            broker.subscribe("s1", (err, subscription) => {
              ...
              subscription
                ...
                .on("invalid_content", (err) => {
                  assert.strictEqual(
                    err.message,
                    "Unknown encryption profile: not-well-known"
                  );
                  done();
                });
            });
        ...
        (err, broker) => {
          ...
          broker.subscribe("s1", (err, subscription) => {
            ...
            subscription.on("cancelled", (err) => {
              assert.strictEqual(
                err.message,
                "Subscription: s1 was cancelled by the broker"
              );
      ...
      createBroker(
        ...
        (err, broker) => {
          ...
          broker.subscribe("s1", (err, subscription) => {
            ...
            subscription.on("error", (err) => {
              assert.ok(
                /Operation failed: BasicConsume; 404 \(NOT-FOUND\)/.test(
                  err.message
                ),
                err.message
              );
