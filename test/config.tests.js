        ...
        configure(
          ...
          (err, config) => {
            ...
            assert.strictEqual(
              ...
              "protocol://user:password@hostname:9000/vhost?heartbeat=10&channelMax=100"
            );
          }
        );
      });
      ...
      it("should decorate the connection config a vhost if not explicitly specified", () => {
        configure(
          ...
          (err, config) => {
            assert.ifError(err);
            assert.strictEqual(
              config.vhosts.v1.connections[0].loggableUrl,
              "protocol://user:***@hostname:9000/v1?heartbeat=10&channelMax=100"
            );
          }
        );
      });
      ...
      it("should set the pathname to empty string if the vhost is /", () => {
        configure(
          ...
          (err, config) => {
            assert.ifError(err);
            assert.strictEqual(
              config.vhosts["/"].connections[0].loggableUrl,
              "protocol://user:***@hostname:9000?heartbeat=10&channelMax=100"
            );
          }
        );
      });
      ...
      it("should decorate the connection config with a loggable url (a)", () => {
        configure(
          ...
          (err, config) => {
            ...
            assert.strictEqual(
              config.vhosts.v1.connections[0].loggableUrl,
              "protocol://user:***@hostname:9000/vhost?heartbeat=10&channelMax=100"
            );
          }
        );
      });
      ...
      it("should generate connections from an array", () => {
        configure(
          ...
          (err, config) => {
            ...
            assert.strictEqual(
              connections[0].url,
              "protocol://user:password@alpha:9000/vhost?heartbeat=10&channelMax=100"
            );
            assert.strictEqual(
              connections[1].url,
              "protocol://user:password@beta:9000/vhost?heartbeat=10&channelMax=100"
            );
          }
        );
      });
        ...
        for (let i = 0; i < 10; i++) {
          configure(
            ...
            (err, config) => {
              assert.ifError(err);
              assert.strictEqual(
                url.parse(config.vhosts.v1.connections[0].url).host,
                url.parse(config.vhosts.v2.connections[0].url).host
              );
              assert.strictEqual(
                url.parse(config.vhosts.v1.connections[0].url).host,
                url.parse(config.vhosts.v3.connections[0].url).host
              );
              assert.strictEqual(
                url.parse(config.vhosts.v1.connections[1].url).host,
                url.parse(config.vhosts.v2.connections[1].url).host
              );
              assert.strictEqual(
                url.parse(config.vhosts.v1.connections[1].url).host,
                url.parse(config.vhosts.v3.connections[1].url).host
              );
              assert.strictEqual(
                url.parse(config.vhosts.v1.connections[2].url).host,
                url.parse(config.vhosts.v2.connections[2].url).host
              );
              assert.strictEqual(
                url.parse(config.vhosts.v1.connections[2].url).host,
                url.parse(config.vhosts.v3.connections[2].url).host
              );
              assert.strictEqual(
                url.parse(config.vhosts.v1.connections[3].url).host,
                url.parse(config.vhosts.v2.connections[3].url).host
              );
              assert.strictEqual(
                url.parse(config.vhosts.v1.connections[3].url).host,
                url.parse(config.vhosts.v3.connections[3].url).host
              );
      ...
      it("should configure the management connection from an object", () => {
        configure(
          ...
          (err, config) => {
            assert.ifError(err);
            assert.strictEqual(
              config.vhosts.v1.connections[0].management.url,
              "https://admin:adminpassword@hostname:9999"
            );
          }
        );
      });
      ...
      it("should configure the management connection with connection credentials", () => {
        configure(
          ...
          (err, config) => {
            ...
            assert.strictEqual(
              config.vhosts.v1.connections[0].management.url,
              "https://user:password@hostname:9999"
            );
          }
        );
      });
      ...
      it("should generate a namespace when specified", () => {
        configure(
          ...
          (err, config) => {
            ...
            assert.ok(
              /\w+-\w+-\w+-\w+-\w+/.test(config.vhosts.v1.namespace),
              format(
                "%s failed to match expected pattern",
                config.vhosts.v1.namespace
              )
            );
            assert.ok(
              /\w+-\w+-\w+-\w+-\w+/.test(config.vhosts.v2.namespace),
              format(
                "%s failed to match expected pattern",
                config.vhosts.v1.namespace
              )
            ...
            assert.ok(
              config.vhosts.v1.namespace !== config.vhosts.v2.namespace
            );
          }
        );
      });
      ...
      it("should not overwrite existing default exchange", () => {
        configure(
          ...
          (err, config) => {
            assert.ifError(err);
            assert.strictEqual(
              config.vhosts.v1.exchanges[""].type,
              "not-overwritten"
            );
          }
        );
      ...
      it("should decorate exchanges with name and fully qualified name", () => {
        configure(
          ...
          (err, config) => {
            ...
            assert.strictEqual(
              config.vhosts.v1.exchanges.e1.fullyQualifiedName,
              "e1"
            );
          }
        );
      ...
      it("should support array of names configuration", () => {
        configure(
          ...
          (err, config) => {
            ...
            assert.strictEqual(
              config.vhosts.v1.exchanges.e2.fullyQualifiedName,
              "e2"
            );
          }
        );
      ...
      it("should decorate queues with name and fully qualified name", () => {
        configure(
          ...
          (err, config) => {
            ...
            assert.strictEqual(
              config.vhosts.v1.queues.q1.fullyQualifiedName,
              "q1"
            );
          }
        );
      ...
      it("should support a mixed array of names / objects configuration", () => {
        configure(
          ...
          (err, config) => {
            ...
            assert.strictEqual(
              config.vhosts.v1.queues.q2.fullyQualifiedName,
              "q2"
            );
          }
        );
      ...
      it('should convert "source -> destination" from binding key', () => {
        configure(
          ...
          (err, config) => {
            assert.ifError(err);
            assert.strictEqual(
              config.vhosts.v1.bindings["e1 -> q1"].source,
              "e1"
            );
            assert.strictEqual(
              config.vhosts.v1.bindings["e1 -> q1"].destination,
              "q1"
            );
            assert.strictEqual(
              config.vhosts.v1.bindings["e:1 -> q:1"].source,
              "e:1"
            );
            assert.strictEqual(
              config.vhosts.v1.bindings["e:1 -> q:1"].destination,
              "q:1"
            );
            assert.strictEqual(
              config.vhosts.v1.bindings["e_1 -> q_1"].source,
              "e_1"
            );
            assert.strictEqual(
              config.vhosts.v1.bindings["e_1 -> q_1"].destination,
              "q_1"
            );
      ...
      it('should convert "source[binding.key] -> destination" from binding key', () => {
        configure(
          ...
          (err, config) => {
            assert.ifError(err);
            assert.strictEqual(
              config.vhosts.v1.bindings["e1[a] -> q1"].bindingKey,
              "a"
            );
            assert.strictEqual(
              config.vhosts.v1.bindings["e1[a.b] -> q1"].bindingKey,
              "a.b"
            );
            assert.strictEqual(
              config.vhosts.v1.bindings["e1[a,a.b] -> q1:a"].bindingKey,
              "a"
            );
            assert.strictEqual(
              config.vhosts.v1.bindings["e1[a,a.b] -> q1:a.b"].bindingKey,
              "a.b"
            );
          }
        );
      ...
      it("should qualify bindings keys when specified", () => {
        configure(
          ...
          (err, config) => {
            ...
            assert.strictEqual(
              config.vhosts.v1.bindings["e1[q1] -> q1"].bindingKey,
              "ns1:q1"
            );
          }
        );
      ...
      it("should configure multiple bindings from an array of binding keys", () => {
        configure(
          ...
          (err, config) => {
            ...
            assert.strictEqual(
              config.vhosts.v1.bindings["b1:a"].destination,
              "q1"
            );
            assert.strictEqual(
              config.vhosts.v1.bindings["b1:a"].bindingKey,
              "a"
            );
            assert.strictEqual(config.vhosts.v1.bindings["b1:b"].source, "e1");
            assert.strictEqual(
              config.vhosts.v1.bindings["b1:b"].destination,
              "q1"
            );
            assert.strictEqual(
              config.vhosts.v1.bindings["b1:b"].bindingKey,
              "b"
            );
          }
        );
      ...
      it("should support a mixed array of names / objects configuration", () => {
        configure(
          ...
          (err, config) => {
            ...
            assert.strictEqual(
              config.vhosts.v1.bindings["e1 -> q2"].source,
              "e1"
            );
            assert.strictEqual(
              config.vhosts.v1.bindings["e1 -> q2"].destination,
              "q2"
            );
          }
        );
    ...
    it("should replace destination its fully qualified names", () => {
      configure(
        ...
        (err, config) => {
          ...
          assert.ok(
            /foo:q1:\w+-\w+-\w+-\w+-\w+/.test(
              config.publications.p2.destination
            ),
            format(
              "%s failed to match expected pattern",
              config.publications.p2.destination
    ...
    it("should create a default publication for each exchange", () => {
      configure(
        ...
        (err, config) => {
          ...
          assert.strictEqual(
            config.publications["v1/e1"].deprecated,
            undefined
          );
        }
      );
    ...
    it("should create a default publication for each queue", () => {
      configure(
        ...
        (err, config) => {
          ...
          assert.strictEqual(
            config.publications["v1/q1"].deprecated,
            undefined
          );
        }
      );
    ...
    it("should create a default subscription for each queue", () => {
      configure(
        ...
        (err, config) => {
          ...
          assert.strictEqual(
            config.subscriptions["v1/q1"].deprecated,
            undefined
          );
        }
      );
    ...
    it("should hoist referenced encryption profiles", (test, done) => {
      configure(
        ...
        (err, config) => {
          assert.ifError(err);
          assert.strictEqual(
            config.publications.p1.encryption.name,
            "well-known"
          );
          ...
          assert.strictEqual(
            config.publications.p1.encryption.algorithm,
            "algo"
          );
          done();
        }
    ...
    it("should replace source with its fully qualified name", () => {
      configure(
        ...
        (err, config) => {
          ...
          assert.strictEqual(config.subscriptions.s1.name, "s1");
          assert.ok(
            /foo:q1:\w+-\w+-\w+-\w+-\w+/.test(config.subscriptions.s1.source),
            format(
              "%s failed to match expected pattern",
              config.subscriptions.s1.source
            )
          );
        }
      );
    ...
    it("should hoist referenced encryption profiles", (test, done) => {
      configure(
        ...
        (err, config) => {
          assert.ifError(err);
          assert.strictEqual(
            config.subscriptions.s1.encryption["well-known"].key,
            "key"
          );
          assert.strictEqual(
            config.subscriptions.s1.encryption["well-known"].ivLength,
            16
          );
          assert.strictEqual(
            config.subscriptions.s1.encryption["well-known"].algorithm,
            "algo"
          );
          done();
        }
    ...
    it("should decorate counter with name and type", () => {
      configure(
        ...
        (err, config) => {
          ...
          assert.strictEqual(
            config.redeliveries.counters.inMemory.type,
            "inMemory"
          );
        }
      );
