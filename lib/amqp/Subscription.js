...
function Subscription(broker, vhost, subscriptionConfig, counter) {
    ...
    sequentialChannelOperations.push((done) => {
      ...
      vhost.getChannel((err, channel) => {
        ...
        _configureQos(config, channel, (err) => {
          if (err) {
            return _closeChannel(channel, () => {
              debug('Error configuring QOS on channel: %s', channel._rascal_id);
              done(err);
            });
          }
          ...
          channel.consume(config.source, onMessage, config.options, (err, response) => {
            ...
            session._open(channel, response.consumerTag, (err) => {
              if (err) {
                return _closeChannel(channel, () => {
                  debug('Error opening session using channel: %s. %s', channel._rascal_id, err.message);
                  done(err);
                });
              }
              timer.reset();
  ...
  function _closeChannel(channel, next) {
    channel.close((err) => {
      if (err) debug('Error closing channel: %s. %s', channel._rascal_id, err.message);
      next();
    });
  }
