...
function Subscription(vhost, config) {
            ...
            channel.consume(config.source, function(message) {
                ...
                getContent(message, config, function(err, content) {
                    err ? session.emit('error', err)
                        : session.emit('message', decorate(message), content, ackOrNack.bind(null, session, channel, message))
                })
    ...
    function ackOrNack(session, channel, message, err, options) {
        if (arguments.length === 4) return ackOrNack(session, channel, message, err, {})
