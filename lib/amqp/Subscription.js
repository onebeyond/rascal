...
function Subscription(vhost, config) {
    ...
    function ackOrNack(session, message, err, options) {
        if (arguments.length === 2) return ackOrNack(session, message, undefined, {})
        if (arguments.length === 3) return ackOrNack(session, message, err, {})
        if (options.defer) return setTimeout(ackOrNack.bind(null, session, message, err, _.omit(options, 'defer')), options.defer).unref()
