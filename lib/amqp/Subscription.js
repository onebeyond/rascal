...
function Subscription(vhost, config) {
    ...
    function ackOrNack(channel, message, err, options) {
        ...
        if (err && !options.republish) return nack(channel, message, options)
        if (err) return republish(channel, message, options)
        ack(channel, message)
    }
    ...
    function republish(subscriberChannel, message, options) {
        ...
        var publishOptions = _.defaultsDeep(message.properties, { headers: { rascal : { republished: 1 } } }, { headers: message.headers })
