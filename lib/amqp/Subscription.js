...
function Subscription(broker, vhost, config, counter) {
  ...
  function _onMessage(session, config, message) {
    if (!message) return _handleConsumerCancel(session, config.queue);
  ...
  function _handleConsumerCancel(session, queue) {
    var err = new Error(format('Received consumer cancel while subscribed to queue: %s. No more messages will be received.', queue));
    debug(err.message);
    session.close(function(cancelErr) {
      if (cancelErr) debug('Error cancelling subscription: %s', cancelErr);
      if (session.emit('cancel', err)) return;
      if (session.emit('error', err)) return;
    });
  }
