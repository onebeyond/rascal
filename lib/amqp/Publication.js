...
function Publication(getChannelFn, publishFn, vhost, config) {
    ...
    this.publish = function(message, overrides, next) {
        ...
        _publish(content, publishConfig, emitter, next)
        return emitter
    }

    function _publish(content, publishConfig, emitter, next) {
        getChannelFn(vhost, emitter, config, function(err, channel) {
            if (err) return next(err)
            publishFn(channel, content.buffer, publishConfig, function(err) {
                if (err) return next(err)
                channel.close() // Channel will already be closed, reclosing will trigger an error
                reply(publishConfig.options.messageId, next)
            })
        })
    }
