...
module.exports = {
    create: function(broker, vhost, config, next) {
        if (config.exchange && config.confirm) return new Publication(getConfirmChannel, publishToConfirmExchange, broker, vhost, config).init(next)
        if (config.exchange) return new Publication(getChannel, publishToExchange, broker, vhost, config).init(next)
        if (config.queue && config.confirm) return new Publication(getConfirmChannel, sendToConfirmQueue, broker, vhost, config).init(next)
        if (config.queue) return new Publication(getChannel, sendToQueue, broker, vhost, config).init(next)
    }
}

function Publication(getChannelFn, publishFn, broker, vhost, config) {
    ...
    this.forward = function(message, overrides, next) {
        var publishConfig = _.defaultsDeep(overrides, config)
        publishConfig.options = _.defaultsDeep(publishConfig.options, message.properties, { headers: { rascal: {} } })
        publishConfig.options.headers.rascal.forwarded = true
