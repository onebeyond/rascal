...
function Broker(config, components) {
  ...
  this.reply = function(vhostName, message, reply, next) {
    const { replyTo, messageId } = message.properties;
    const options = { routingKey: replyTo, options: { correlationId: messageId } };
    const finalVhostName = /\/$/.test(vhostName) ? vhostName : `${vhostName}/`;
    return this.publish(finalVhostName, reply, options, next);
  };
