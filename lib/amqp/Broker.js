...
var noCache = require('../caches/noCache')
...
function Broker(config, components) {
    ...
    var publications = {}
    var subscriptions = {}
    var init = async.compose(tasks.initShovels, tasks.initSubscriptions, tasks.initPublications, tasks.initCaches, tasks.initVhosts)
    ...
    var caches = _.defaults({}, components.caches, { noCache: noCache, inMemory: inMemory })

    this.config = config

    this.init = function(next) {
        debug('Initialising broker')
        init(config, { broker: self, components: { caches: caches } }, function(err, config, ctx) {
            vhosts = ctx.vhosts
