...
function Broker(config) {
    ...
    this.publish = function(name, message, overrides, next) {
        if (arguments.length < 3) return _publish(name, message, {}, _.noop)
        if (arguments.length === 3 && _.isFunction(arguments[2])) return _publish(name, message, {}, arguments[2])
        if (arguments.length === 3) return _publish(name, message, overrides, _.noop)
        if (_.isString(overrides)) return _publish(name, message, { routingKey: overrides }, next)
        _publish(name, message, overrides, next)
    }
    ...
    this.subscribe = function(name, handler, overrides, next) {
        ...
        _subscribe(name, handle, override, next)
    }
