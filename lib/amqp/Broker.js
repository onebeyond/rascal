...
function Broker(config) {
    ...
    this.subscribe = function(name, handler, overrides, next) {
        ...
        if (arguments.length < 3) return self.subscribe(name, handler, {}, _.noop)
        if (arguments.length === 3 && _.isFunction(arguments[2])) return self.subscribe(name, handler, {}, arguments[2])
        if (arguments.length === 3) return self.subscribe(name, handler, overrides, _.noop)
