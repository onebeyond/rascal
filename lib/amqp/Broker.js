...
function Broker(config, components) {
  ...
  const counters = _.defaults({}, components.counters, { stub: stub, inMemory: inMemory, inMemoryCluster: inMemoryCluster });
  ...
  this._init = function(next) {
    ...
    subscriptions = {};
    sessions = [];
    init(config, { broker: self, components: { counters: counters } }, function(err) {
  ...
  this.shutdown = function(next) {
    ...
    async.eachSeries(_.values(vhosts), function(vhost, callback) {
      forewarnVhost(config, { vhost: vhost }, callback);
    }, function(err) {
      if (err) return next(err);
      self.unsubscribeAll(function(err) {
        if (err) return next(err);
