...
module.exports = _.curry(function(config, ctx, next) {
  ...
  if (!_.has(ctx, 'connectionIndex')) ctx.connectionIndex = 0;
  var candidates = config.connections;

  async.retry(candidates.length, function(cb) {
    var connectionIndex = ctx.connectionIndex++ % candidates.length;
    var connectionConfig = candidates[connectionIndex];
