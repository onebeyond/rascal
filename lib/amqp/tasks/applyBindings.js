...
function bindQueue(config, channel, binding, next) {
    ...
    channel.bindQueue(destination.fullyQualifiedName, source.fullyQualifiedName, binding.routingKey, binding.options, next)
}
...
function bindExchange(config, channel, name, next) {
    ...
    channel.bindExchange(destination.fullyQualifiedName, source.fullyQualifiedName, binding.routingKey, binding.options, next)
}