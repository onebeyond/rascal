...
var negotiateContent = require('../utils/negotiateContent');
...
function Vhost(config) {
  ...
  this.addReplyHandler = function(messageId, handler) {
    replyHandlers[messageId] = handler;
  };

  this.getReplyHandler = function(messageId) {
    return replyHandlers[messageId];
  };

  this.deleteReplyHandler = function(messageId) {
    delete replyHandlers[messageId];
  };

  function createChannelPool(options) {
    var poolOptions = _.defaultsDeep({}, { idleTimeoutMillis: 0, evictionRunIntervalMillis: 0 }, options.pool);
    var pool, poolQueue;
    ...
    debug('Creating %s channel pool %o', mode, poolOptions);
    ...
    function callback(err, channel) {
      ...
      channel.consume(
        ...
        function(message) {
          ...
          const replyHandler = self.getReplyHandler(correlationId);

          if (!replyHandler) return;

          negotiateContent(message.properties.contentType, message.content, (error, content) => {
            if (error) return replyHandler.emit('error', error);
            replyHandler.emit('message', message, content);
          });
        },
        { noAck: true },
        function(err) {
