...
function Vhost(config) {
  ...
  this.nuke = function(next) {
    ...
    drainChannelPools(function(err) {
      if (err) return next(err);
      nuke(config, { connectionIndex: self.connectionIndex }, function(err) {
        if (err) return next(err);
  ...
  this.purge = function(next) {
    ...
    purge(config, { purge: true, connectionIndex: self.connectionIndex }, function(err) {
      if (err) return next(err);
      ...
      create() {
        return new Promise(function(resolve, reject) {
          ...
          createChannelWhenInitialised(options.confirm, function(err, channel) {
            ...
            const destroyChannel = _.once(function() {
