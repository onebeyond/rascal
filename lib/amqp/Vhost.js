...
function Vhost(config) {
    ...
    var confirmChannelPool = createChannelPool({ confirm: true, size: config.publicationChannelPools.confirmSize })
    ...
    this.init = function(next) {
        ...
        init(config, {}, function(err, config, ctx) {
            ...
            ctx.connection.once('close', handleConnectionError.bind(null, config))
    ...
    function createChannelPool(options) {
        ...
        function borrow(next) {
            ...
            poolQueue.push(null, function (err, channel) {
                if (err) return next(err);
                debug('Borrowed%s channel. %s', displayType, stats())
                next(null, channel)
            })
        }

        function release(channel) {
            debug('Returning%s channel. %s', displayType, stats())
            pool.release(channel)
            debug('Returned%s channel. %s', displayType, stats())
        }
    ...
    function handleConnectionError(config, err) {
        debug('Handling connection error: %s from %s', err.message, connectionConfig.loggableUrl)
