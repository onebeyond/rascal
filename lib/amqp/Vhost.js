...
function Vhost(config) {
  ...
  function createChannelPool(options) {
    var pool, poolQueue;
    var displayType = options.confirm ? 'confirm' : 'normal';
    var busy = false;
    var poolOptions = {
      max: options.size,
      min: 0,
            ...
            var destroyChannel = _.once(function() {
              ...
              if (pool.isBorrowedResource(channel)) {
                pool.destroy(channel).catch(function(err) {
                  debug('Error destroying %s channel: %s. %s', displayType, err.message);
                });
              }
            });
    ...
    function borrow(next) {
      debug('Requested %s channel. %o', displayType, stats());
      if (poolQueue.length() >= poolOptions.max) {
        busy = true;
        self.emit('busy', stats());
      }
      poolQueue.push(null, function (err, channel) {
        if (err) return next(err);
        debug('Borrowed %s channel: %s. %o', displayType, channel._rascal_id, stats());
        next(null, channel);
      });
    }

    function release(channel) {
      debug('Releasing %s channel: %s. %o', displayType, channel._rascal_id, stats());
      pool.release(channel).then(function() {
        if (poolQueue.length() > 0 || !busy) return;
        busy = false;
        self.emit('ready', stats());
      }).catch(function(err) {
        debug('Error releasing %s channel: %s. %s', displayType, err.message);
      });
    }

    function drain(next) {
      debug('Draining %s channel pool. %o', displayType, stats());
      pool.drain().then(function() {
        return pool.clear().then(function() {
          debug('Drained %s channel pool. %o', displayType, stats());
          setImmediate(next);
        });
      }).catch(function(err) {
        debug('Error draining %s channel pool. %s', displayType, err.message);
        setImmediate(next);
      });
    }

    debug('Creating %s channel pool with %d channels', displayType, options.size);
    pool = genericPool.createPool(factory, poolOptions);
