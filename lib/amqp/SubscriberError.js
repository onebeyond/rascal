...
module.exports = function SubscriptionRecovery(broker, vhost) {
    ...
    this.handle = function(session, message, recoveryProcess, next) {
        ...
        async.eachSeries([].concat(recoveryProcess), function(recoveryConfig, cb) {
            ...
            setTimeout(function() {
                getStrategy(recoveryConfig).execute(session, message, _.omit(recoveryConfig, 'defer'), function(err, handled) {
    ...
    var recoveryStrategies = _.indexBy([
        ...
        },
        {
            ...
            execute: function(session, message, strategyConfig, next) {
                ...
                broker.forward(strategyConfig.publication, message, strategyConfig.options || {}, function(err, publication) {
                    if (err) return next(err)
                    publication.on('success', function() {
                        debug('Message: %s was forwarded to publicaiton: %s %d times', message.properties.messageId, strategyConfig.publication, forwarded + 1)
                        session._ack(message)
                        next(null, true)
                    })
                    .on('error', next)
        ...
        },
        {
            name: 'unknown',
            execute: function(session, message, strategyConfig, next) {
                session.emit('error', new Error(format('Error recovering message: %s. No such strategy: %s.', message.properties.messageId, strategyConfig.strategy)))
                next()
            }
        }
    ], 'name')
