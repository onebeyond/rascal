      ...
      },
      {
        ...
        execute(session, message, err, strategyConfig, next) {
          ...
          vhost.getConfirmChannel((err, publisherChannel) => {
            ...
            if (!publisherChannel) return once(new Error('Unable to handle subscriber error by republishing. The VHost is shutting down'));

            publisherChannel.on('error', once);
            publisherChannel.on('return', () => {
              once(new Error(format('Message: %s was forwared to queue: %s, but was returned', message.properties.messageId, originalQueue)));
            });

            publisherChannel.publish(undefined, originalQueue, message.content, publishOptions, (err) => {
              if (err) return once(err); // Channel will already be closed, reclosing will trigger an error
      ...
      },
      {
        ...
        execute(session, message, err, strategyConfig, next) {
          ...
          broker.forward(strategyConfig.publication, message, forwardOverrides, (err, publication) => {
            ...
            publication.on('error', once);
            publication.on('return', () => {
              once(new Error(format('Message: %s was forwared to publication: %s, but was returned', message.properties.messageId, strategyConfig.publication)));
            });
          });
        },
      },
