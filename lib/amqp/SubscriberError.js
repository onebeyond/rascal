    ...
    var recoveryStrategies = _.indexBy([
        ...
        },
        {
            ...
            execute: function(session, message, strategyConfig, next) {
                var nacked = message.properties.headers['x-death'] && message.properties.headers['x-death'][0].count || 0

                if (strategyConfig.options && strategyConfig.options.attempts && strategyConfig.options.attempts <= nacked) {
                    debug('Message: %s has been nacked %d times.', message.properties.messageId, nacked)
        ...
        },
        {
            ...
            execute: function(session, message, strategyConfig, next) {
                ...
                vhost.getConfirmChannel(function(err, publisherChannel) {
                    ...
                    // TODO tunnel channel errors through to session emitter
