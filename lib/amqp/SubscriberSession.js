...
function SubscriberSession(options) {
    ...
    this._ack = function(message) {
        withConsumerChannel(message.fields.consumerTag, function(channel) {
            ...
            self.emit('error', new Error('The channel has been closed. Unable to ack message'))
        })
    }
    ...
    this._nack = function(message, options) {
        ...
        withConsumerChannel(message.fields.consumerTag, function(channel) {
            ...
            self.emit('error', new Error('The channel has been closed. Unable to nack message'))
        })
    }
    ...
    */
    function scheduleClose(entry) {
        debug('Defering close channel: %s by %dms', entry.channel._rascal_id, options.deferCloseChannel)
        setTimeout(function() {
