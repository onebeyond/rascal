    ...
    */
    function doom(consumerTag) {
        withConsumerChannel(consumerTag, function(channel, consumerTag, entry) {
            ...
            debug('Dooming channel: %s', channel._rascal_id)
            entry.doomed = true
            scheduleClose(consumerTag)
        })
    }

    function scheduleClose(consumerTag) {
        setTimeout(function() {
            withConsumerChannel(consumerTag, function(channel) {
                channel.close(function() {
                    debug('Doomed channel: %s was closed', channel._rascal_id)
                })
            })
        }, 60000).unref()
    }
