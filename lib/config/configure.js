...
function configureBindings(config) {
    _.each(expandBindings(config.bindings), function(bindingConfig, name) {
...
function expandBindings(definitions) {
    ...
    _.each(definitions, function(bindingConfig, name) {
        ...
        if (bindingKeys.length <= 1) return bindings[name] = bindingConfig
        _.each(bindingKeys, function(bindingKey) {
            bindings[format('%s:%s', name, bindingKey)] = _({ bindingKey: bindingKey }).defaults(bindingConfig).omit('bindingKeys').value()
        })
    })
    return bindings
}
