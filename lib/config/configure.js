...
module.exports = _.curry((rascalConfig, next) => {
  ...
  function parseConnectionUrl(connectionString) {
    ...
    const { protocol, username: user, password, hostname, port, pathname: vhost, searchParams } = new URL(connectionString);
    const options = Array.from(searchParams).reduce((attributes, entry) => ({ ...attributes, [entry[0]]: entry[1] }), {});
    return { protocol, hostname, port, user, password, vhost, options };
  }

  function getConnectionAttributes(attributes) {
    const { protocol, hostname, port, user, password, vhost, options, socketOptions, management } = attributes;
    return { protocol, hostname, port, user, password, vhost, options, socketOptions, management };
  }
