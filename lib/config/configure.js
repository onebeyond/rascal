...
function configureExchanges(config) {
    _.each(config.exchanges, function(exchangeConfig, name) {
        ...
        config.exchanges[name] = _.defaultsDeep(exchangeConfig, { name: name, fullyQualifiedName: qualify(name, config.namespace) }, config.defaults.exchanges)
    })
}
...
function configureQueues(config) {
    _.each(config.queues, function(queueConfig, name) {
        ...
        config.queues[name] = _.defaultsDeep(queueConfig, { name: name, fullyQualifiedName: qualify(name, config.namespace, queueConfig.replyTo) }, config.defaults.queues)
    })
}
...
function configureBindings(config) {
    ...
    _.each(config.bindings, function(bindingConfig, name) {
        ...
        config.bindings[name] = _.defaultsDeep(bindingConfig, { name: name }, config.defaults.bindings)
        if (bindingConfig.destinationType === 'queue') {
            config.bindings[name].bindingKey = prefix(config.queues[bindingConfig.destination].replyTo, bindingConfig.bindingKey, '.')
        }
    })
}
...
function expandBindings(definitions) {
    _.each(definitions, function(bindingConfig, name) {
        ...
        var bindingKeys = _.chain([]).concat(bindingConfig.bindingKeys, bindingConfig.bindingKey).compact().value()
        if (bindingKeys.length <= 1) return definitions[name] = _({ bindingKey: bindingKeys[0] }).defaults(bindingConfig).omit('bindingKeys').value()
        _.each(bindingKeys, function(bindingKey) {
            definitions[format('%s:%s', name, bindingKey)] = _({ bindingKey: bindingKey }).defaults(bindingConfig).omit('bindingKeys').value()
        })
    })
}
...
function qualifyArguments(namespace, args) {
    ...
    _.each(['x-dead-letter-exchange'], function(name) {
        args[name] = args[name] !== undefined ? qualify(args[name], namespace) : args[name]
    })
}
