...
module.exports = _.curry(function(rascalConfig, next) {
    ...
    try {
        ...
        configureCaches(rascalConfig.redeliveries.caches)
    ...
    function configureCaches(caches) {
        rascalConfig.caches = ensureKeyedCollection(caches)
        _.each(rascalConfig.caches, configureCache)
    }

    function configureCache(cacheConfig, name) {
        debug('Configuring cache: %s', name)
        rascalConfig.caches[name] = _.defaultsDeep(cacheConfig, { name: name, type: name }, rascalConfig.defaults.caches)
    }
})
