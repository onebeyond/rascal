...
module.exports = _.curry(function(rascalConfig, next) {
    ...
    function configureConnections(vhostConfig, vhostName) {
        vhostConfig.connections = _.chain([]).concat(vhostConfig.connections, vhostConfig.connection).compact().unique().map(function(connection) {
    ...
    function configureVhostPublications(vhostConfig) {
        _.each(_.defaults({}, vhostConfig.publications, defaultPublications), function(publicationConfig, name) {
    ...
    function configurePublications(publications, vhosts) {
        var defaultPublications = _.reduce(vhosts, function(publications, vhost) {
            _.each(vhost.exchanges, function(exchange) {
                publications[exchange.name] = { exchange: exchange.name, vhost: vhost.name }
            })
            _.each(vhost.queues, function(queue) {
                publications[queue.name] = { queue: queue.name, vhost: vhost.name }
            })
            return publications
        }, {})
