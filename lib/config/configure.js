...
module.exports = _.curry(function(rascalConfig, next) {
  ...
  function configureVhosts(vhosts) {
    _.each(vhosts, function(vhostConfig, name) {
  ...
  function configurePublications(publications, vhosts) {
    const defaultPublications = _.reduce(vhosts, function(publications, vhost) {
  ...
  function configureSubscriptions(subscriptions, vhosts) {
    const defaultSubscriptions = _.reduce(vhosts, function(subscriptions, vhost) {
      _.each(vhost.queues, function(queue) {
        const name = vhost.name === '/' ? format('/%s', queue.name) : format('%s/%s', vhost.name, queue.name);
  ...
  function parseShovelName(name) {
    ...
    return match ? { name, subscription: match.subscription, publication: match.publication } : { name };
  }
  ...
  function parseBindingName(name) {
    ...
    const match = XRegExp.exec(name, pattern);
    return match ? { name, source: match.source, destination: match.destination, bindingKeys: splitBindingKeys(match.keys) } : { name };
  }
