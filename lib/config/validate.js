...
module.exports = _.curry(function(config, next) {
  ...
  function validateVhost(vhost, vhostName) {
    validateAttributes('Vhost', vhost, vhostName, ['defaults', 'namespace', 'name', 'publicationChannelPools', 'connection', 'connections', 'exchanges', 'queues', 'bindings', 'check', 'assert']);
  ...
  function validateSubscription(subscription, subscriptionName) {
    validateAttributes('Subscription', subscription, subscriptionName, ['name', 'vhost', 'queue', 'contentType', 'options', 'prefetch', 'retry', 'source', 'recovery', 'workflow', 'handler', 'workflows', 'handlers', 'redeliveries', 'autoCreated', 'deprecated', 'deferCloseChannel', 'encryption']);
