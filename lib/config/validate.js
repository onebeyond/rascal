...
module.exports = _.curry(function(config, next) {
    ...
    function validateSubscription(subscription, subscriptionName) {
        ...
        if (!config.redeliveries.caches[subscription.redeliveries.cache]) throw new Error(format('Subscription: %s refers to an unknown cache: %s in vhost: %s', subscriptionName, subscription.redeliveries.cache, subscription.vhost))
    }
