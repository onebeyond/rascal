...
var debug = require('debug')('amqp-nice:config:validate')
...
module.exports = _.curry(function(config, next) {
    ...
    function validateVhost(vhost, vhostName) {
        validateAttributes('Vhost', vhost, vhostName, ['namespace', 'connection', 'exchanges', 'queues', 'bindings'])
        validateConnectionAttributes(vhost.connection, vhostName, ['slashes', 'protocol', 'hostname', 'user', 'password', 'port', 'vhost', 'options', 'retry'])
    ...
    function validateExchange(vhost, vhostName, exchange, exchangeName) {
        validateVhostChildAttributes(vhostName, 'Exchange', exchange, exchangeName, ['assert', 'check', 'type', 'options'])
    }
    ...
    function validateQueue(vhost, vhostName, queue, queueName) {
        validateVhostChildAttributes(vhostName, 'Queue', queue, queueName, ['assert', 'check', 'type', 'purge', 'options'])
    }
    ...
    function validateBinding(vhost, vhostName, binding, bindingName) {
        validateVhostChildAttributes(vhostName, 'Binding', binding, bindingName, ['source', 'destination', 'destinationType', 'bindingKey'])
    ...
    function validatePublication(publication, publicationName) {
        validateAttributes('Publication', publication, publicationName, ['vhost', 'exchange', 'queue', 'routingKey', 'confirm', 'options'])
    ...
    function validateSubscription(subscription, subscriptionName) {
        validateAttributes('Subscription', subscription, subscriptionName, ['vhost', 'queue', 'contentType', 'options'])
