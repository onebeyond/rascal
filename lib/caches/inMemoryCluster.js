...
var debug = 'rascal:caches:inMemoryCluster'

module.exports = {
    master: function master(options) {
        var workers = {}
        var cache = inMemory(options)

        function handleMessage(worker, message) {
            if (message.sender !== 'rascal-cluster-cache' || message.cmd !== 'incrementAndGet') return
            cache.incrementAndGet(message.key, function(err, value) {
                worker.send({ sender: 'rascal-cluster-cache', correlationId: message.correlationId, value: err ? 1 : value })
            })
        }
    ...
    worker: function worker(options) {
        ...
        process.on('message', function(message) {
            if (message.sender !== 'rascal-cluster-cache') return
